...
function getPrintScreenMessageFromServerByPOST_md5(md5, url, callback, onerror)
{	

	var params = 'md5='+encodeURIComponent(md5);
	var xhr = new XMLHttpRequest();
	xhr.open('POST', url, true);
...
function getPrintScreenMessageFromServerByPOST_md5AndReturnBlob( roomID, url, callback, error )
{	
	...
	xhr.onload = function(e) {  
		...
		if (xhr.status != 200) {  
			var error1 = xhr.status + ': ' + xhr.statusText; 
