function client_save_script( callback )
{
	if (document.getElementById("modal_window0")==null)
	{   global_client_typing_mode = true;
		modal_window( hsave_script, callback );
	}
}


function hsave_script(el)
{
	/********
	...
	*******/		
			...
			execute_t_script_MSC(all_commands, function()
			{
				global_client_typing_mode = false;
				__client_save_script(all_commands, function(){
...
function execute_t_script_MSC(all_commands, callback)
{
	...
	//	console.log(arr);
		...
		setTimeout ( function(){ 
			...
			execute_t_script_MSC(all_commands, callback);
	...
	else if (global_ec_vars_arr_index < global_ec_vars_arr_length)
	{
		...
		execute_instruction_MSC ( global_ec_vars_arr[global_ec_vars_arr_index], function(){ 
		
			console.log(''+global_ec_vars_arr[global_ec_vars_arr_index]);
			global_ec_vars_arr_index++;
			
			// setTimeout ( execute_t_script_MSC, 500 );
...
function execute_instruction_MSC ( instr, callback)
{
	instr = instr.trim();
	//instr = instr.replace(/%!%!%/g,",");
	
	var params=[];
	var arr = dvummi_MSC(instr);
	if(arr==null)
	{
...
/////////////////////////////////////////
function __client_save_script(all_commands, callback ) {
	console.log("__client_save_script: "+ all_commands);
	callback();
}
